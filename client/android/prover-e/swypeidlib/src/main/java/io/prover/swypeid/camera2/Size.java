...
public class Size implements Parcelable {
    ...
    public static List<Size> fromCamera2SizeList(List<android.util.Size> source) {
        List<Size> result = new ArrayList<>();
        for (android.util.Size size : source) {
            result.add(new Size(size));
        }
        return result;
    }

    public static Size[] fromCamera2SizeArray(android.util.Size[] source) {
        Size[] result = new Size[source.length];
        for (int i = 0; i < result.length; i++) {
            result[i] = new Size(source[i]);
        }
        return result;
    }
