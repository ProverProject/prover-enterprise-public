...
class SwypeDetector {
    ...
    private func doProcess(_ imageBuffer: CVImageBuffer, timestamp: CMTime) {
        ...
        if state != prevState {
            ...
            stateDelegate.detectorDidChangeState(to: detectorState)
        ...
        if state == SwypeDetectorState.detectingSwypeCode.rawValue {
            ...
            if state != prevState || index != prevIndex {
                ...
                stateDelegate.detectorDidChangeIndex(to: Int(index))
                coordinateDelegate.detectorDidChangeDirection(to: currentDirection)
            }
            else {
                if let direction = currentDirection {
                    let x = CGFloat(direction.x) - CGFloat(xValue) / CGFloat(1024)
                    let y = CGFloat(direction.y) - CGFloat(yValue) / CGFloat(1024)
