...
extension VideoRecorder {
    ...
    func startSession() {
        ...
        captureSession.sessionPreset = Settings.currentVideoCapturePreset
...
extension VideoRecorder {
    ...
    func startRecord() {
        ...
        let assistant = AVOutputSettingsAssistant(preset: Settings.currentVideoOutputPreset)!
    ...
    private func exportMetadata() {
        ...
        let detectorXScale = delegate.detectorXScale
        let detectorYScale = delegate.detectorYScale
        ...
        proverValue += "version:\(delegate.getSwypeHelperVersion());"
        ...
        let minFrameTime = delegate.getMinProcessingTime()
        let maxFrameTime = delegate.getMaxProcessingTime()
        let avgFrameTime = String(format: "%.2f", Double(delegate.getTotalTime()) / Double(delegate.getTotalFrames()))
        ...
        assetExportSession.exportAsynchronously { [unowned self] in
